class dsDOM {
        ...
        if (typeof selector === 'string') {
            if (selector.startsWith('<') && selector.endsWith('>')) {
                if(!this._selectorCache.has(selector)) {
                    ...
                    return this._selectorCache.set(selector, [...tamplate.content.childNodes])
                }
    ...
    listen(event, selector, handler) {
        for(let i = 0, n = this.elements.length; i < n; i++) {
            ...
            // eventCache[event].push({selector, handler: delegatingHandler})
            // eventCache[event] = true
            eventCache[cacheKey] = delegatingHandler
        }
        return this
    }
