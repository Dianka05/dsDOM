class dsDOM extends Array {
    ...
    listen(event, selector, handler) {
        ...
        const handlerKey = isFn ? handler.toString() : handler
        ...
        for(let i = 0, n = this.length; i < n; i++) {
            ...
            const key = `${event}_${selector || '___'}_${handlerKey}`
            if (eventCache[key] !== undefined) continue
            
            const delegatingHandler = hasSelector
                ? function(e) {
                    for (let t = e.target; t && t !== el; t = t.parentNode) { 
            ...
            eventCache[key] = delegatingHandler
